...
function PacmanControls(): ReactElement {
  ...
  const pacmanEncounterWithGhost = (charactersState: CharacterStateType) => {
    // setGameOver(true);
    debugger
  }
  ...
  useEffect(function gameTick() {
    ...
    const timer = setInterval(() => {
      ...
      if ((lastTime + (config.pacmanSpeed * 1000)) < timeNow && gameOver === false) {
        setLastTime(timeNow);
        handleGameTick();
      }
    }, timeOutSpeed)
    return () => clearInterval(timer)
  }, [fullMazeState, lastTime, gameOver]);
